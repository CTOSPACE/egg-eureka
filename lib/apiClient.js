...
class APIClient extends APIClientBase {
  ...
  client (key) {
    ...
    const cacheKey = serverUrl + '@' + key
    let breakerRequest = this._breakerRequestMap[cacheKey]
    if (!breakerRequest) {
      breakerRequest = new BreakerRequest(serverUrl, {
        circuitBreakerConfig: this.circuitBreakerConfig,
        logger: this.logger,
        curl: this.curl
      })
      this._breakerRequestMap[cacheKey] = breakerRequest
    }
    return breakerRequest
  }
}
